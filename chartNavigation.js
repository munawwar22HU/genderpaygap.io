...
function addScrollWheelNavigation() {
  ...
  window.addEventListener(
    "wheel",
    function (event) {
      // Check if the overlay (popup-content) is open
      const overlay = document.querySelector(".overlay");
      if (overlay && overlay.classList.contains("visible")) {
        return; // Ignore scroll events when popup-content is open
      }
...
function infoBoxScroll() {
  const infoBox = document.querySelector(".overlay");

  infoBox.addEventListener("wheel", function (event) {
    const containerElement = this.querySelector(".instructions-popup"); // The element to scroll inside overlay

    // Check if the container is scrollable (scrollHeight > clientHeight means content is taller than visible area)
    if (containerElement.scrollHeight > containerElement.clientHeight) {
      // Prevent the main page scroll listener from firing
      event.stopPropagation();

      // Adjust the container's scroll position (deltaY is positive for scrolling down, negative for up)
      containerElement.scrollTop += event.deltaY;

      // Optional: Prevent the default scrolling behavior of the browser (if needed)
