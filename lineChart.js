...
function drawLineChart(yearlyData) {
  ...
  const gapPath = g
    ...
    .attr("stroke", "black")
    .attr("stroke-width", 2)
    .style("stroke-dasharray", "5,5") // define dash pattern
    .attr("d", gapLine);

  const gapPathLength = gapPath.node().getTotalLength();
  gapPath
    .attr("stroke-dasharray", gapPathLength) // for animation length
  ...
  // Specific mouseover for Women
  const mouseoverGapPercentage = function (event, d) {
    tooltip
      .html(
        `<strong>Year:</strong> ${d.year}<br/>` +
          `<strong>Gap Percentage:</strong> ${d.gapPercentage.toLocaleString()}%`
      )
      .classed("visible", true);
  };
  ...
  g.selectAll(".wage-dot")
    ...
    .attr("fill", "black")
    .style("cursor", "pointer")
    .on("mouseover", mouseoverGapPercentage) // Use specific handler
    .on("mousemove", mousemove) // Use generic handler
    .on("mouseout", mouseout); // Use generic handler
