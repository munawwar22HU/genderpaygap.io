...
function drawLineChart(yearlyData) {
  ...
  // Add lines with animation - updated with new colors
  const menPath = g.append("path")
    .datum(yearlyData)
    .attr("class", "men-line")
    .attr("fill", "none")
    .attr("stroke", CHART_COLORS.PRIMARY_1) // Updated color
    .attr("stroke-width", 2.5)
    .attr("d", menLine);
  
  const menPathLength = menPath.node().getTotalLength();
  menPath.attr("stroke-dasharray", menPathLength)
  ...
  const womenPath = g.append("path")
    ...
    .attr("fill", "none")
    .attr("stroke", CHART_COLORS.PRIMARY_2) // Updated color
    .attr("stroke-width", 2.5)
    .attr("d", womenLine);
  
  const womenPathLength = womenPath.node().getTotalLength();
  womenPath.attr("stroke-dasharray", womenPathLength)
  ...
  const gapPath = g.append("path")
    ...
    .attr("class", "gap-line")
    .attr("fill", "none")
    .attr("stroke", CHART_COLORS.ACCENT) // Updated color to amber
    .attr("stroke-width", 2)
    .attr("stroke-dasharray", "5,5")
    .attr("d", gapLine);
  
  const gapPathLength = gapPath.node().getTotalLength();
  gapPath.attr("stroke-dasharray", `5,5,${gapPathLength}`)
  ...
  // Add circles for men's wage data points with tooltip events - updated colors
  g.selectAll(".men-dot")
    .data(yearlyData)
    .join("circle")
    .attr("class", "men-dot")
  ...
  // Add circles for women's wage data points with tooltip events - updated colors
