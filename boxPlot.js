function drawHoursBoxPlot(hoursData) {
  ...
  boxGroups.each(function(d) {
      ...
      const baseColor = d.category.includes('Male') ? "#2563EB" : "#DB2777";
      ...
      // Median wage annotation (Optional)
      currentGroup.append("text")
        .attr("class", "wage-annotation")
        .attr("x", x(d.max) + 15)
        .attr("y", boxHeight / 2) // Relative position
        .attr("font-size", "11px")
        .attr("fill", "#444")
  ...
  legend.append("rect").attr("x", 0).attr("y", 0).attr("width", 12).attr("height", 12).attr("fill", "#2563EB").attr("opacity", 0.2).attr("stroke", "#2563EB");
  legend.append("text").attr("x", 18).attr("y", 10).text("Men");
  legend.append("rect").attr("x", 120).attr("y", 0).attr("width", 12).attr("height", 12).attr("fill", "#DB2777").attr("opacity", 0.2).attr("stroke", "#DB2777");
