...
function drawAgeVerticalBarChart(ageData) {
  ...
  const mouseover = function(event, d) {
    tooltip
     ...
     // d3.select(this).style("opacity", 0.7); // Rely on CSS hover effect instead
  };
  ...
  const mouseout = function(event, d) {
    ...
    //d3.select(this).style("opacity", 1); // Rely on CSS hover effect instead
  };
  // --- End Tooltip Handlers ---

  // Men's bars with animation and tooltip events
  g.selectAll(".men-bar")
    .data(ageData)
    .join("rect") // Use .join()
    .attr("class", "bar men-bar")
    .attr("x", (d) => xScale(d.ageGroup) + xSubgroup("men"))
    .attr("y", innerHeight) // Start for animation
    .attr("width", xSubgroup.bandwidth())
    .attr("height", 0) // Start for animation
    .attr("fill", "#2563EB")
  ...
  // Women's bars with animation and tooltip events
...
function drawRaceVerticalBarChart(raceData) {
  ...
  // --- Tooltip Handlers for Race ---
   const mouseoverRace = function(event, d) {
    tooltip
      .html(
  ...
  const mousemoveRace = function(event, d) {
    tooltip
      .style("left", (event.pageX + 15) + "px")
      .style("top", (event.pageY - 10) + "px");
  };

  const mouseoutRace = function(event, d) {
    tooltip.classed('visible', false);
  ...
  g.selectAll(".men-bar")
    ...
    .attr("fill", "#2563EB")
    .style("cursor", "pointer")
    .on("mouseover", mouseoverRace) // Attach race handlers
    .on("mousemove", mousemoveRace)
    .on("mouseout", mouseoutRace)
    .transition()
    .duration(800)
