...
function drawAgeVerticalBarChart(ageData) {
  ...
  // Men's bars with animation and specific tooltip events
  g.selectAll(".men-bar").data(ageData).join("rect").attr("class", "bar men-bar").attr("x", (d) => xScale(d.ageGroup) + xSubgroup("men")).attr("y", innerHeight).attr("width", xSubgroup.bandwidth()).attr("height", 0).attr("fill", "#2563EB").style("cursor", "pointer")
    .on("mouseover", mouseoverMenAge) // Attach specific handler
    .on("mousemove", mousemove)
    .on("mouseout", mouseout)
    .transition().duration(1000).attr("y", (d) => yScale(d.menWage)).attr("height", (d) => innerHeight - yScale(d.menWage));

  // Women's bars with animation and specific tooltip events
  g.selectAll(".women-bar").data(ageData).join("rect").attr("class", "bar women-bar").attr("x", (d) => xScale(d.ageGroup) + xSubgroup("women")).attr("y", innerHeight).attr("width", xSubgroup.bandwidth()).attr("height", 0).attr("fill", "#DB2777").style("cursor", "pointer")
    .on("mouseover", mouseoverWomenAge) // Attach specific handler
    .on("mousemove", mousemove)
    .on("mouseout", mouseout)
    .transition().duration(1000).attr("y", (d) => yScale(d.womenWage)).attr("height", (d) => innerHeight - yScale(d.womenWage));
...
function drawRaceVerticalBarChart(raceData) {
  ...
  // Title
  g.append("text").attr("class", "chart-title").attr("x", innerWidth / 2).attr("y", -20).attr("text-anchor", "middle").text("Wages by Race/Ethnicity");

  // X axis
  const x = d3.scaleBand().domain(sortedData.map((d) => d.race)).range([0, innerWidth]).padding(0.4);
  g.append("g").attr("transform", `translate(0, ${innerHeight})`).call(d3.axisBottom(x));

  // Y axis
  const y = d3.scaleLinear().domain([0, d3.max(sortedData, (d) => Math.max(d.menWage, d.womenWage)) * 1.1]).range([innerHeight, 0]);
  ...
  // Subgroup scale
  const xSubgroup = d3.scaleBand().domain(["men", "women"]).range([0, x.bandwidth()]).padding(0.3);
  ...
  // Men's bars
  g.selectAll(".men-bar").data(sortedData).join("rect").attr("class", "bar men-bar").attr("x", (d) => x(d.race) + xSubgroup("men")).attr("y", innerHeight).attr("width", xSubgroup.bandwidth()).attr("height", 0).attr("fill", "#2563EB").style("cursor", "pointer")
    .on("mouseover", mouseoverMenRace) // Attach specific handler
    .on("mousemove", mousemove)
    .on("mouseout", mouseout)
    .transition().duration(800).attr("y", (d) => y(d.menWage)).attr("height", (d) => innerHeight - y(d.menWage));

  // Women's bars
  g.selectAll(".women-bar").data(sortedData).join("rect").attr("class", "bar women-bar").attr("x", (d) => x(d.race) + xSubgroup("women")).attr("y", innerHeight).attr("width", xSubgroup.bandwidth()).attr("height", 0).attr("fill", "#DB2777").style("cursor", "pointer")
    .on("mouseover", mouseoverWomenRace) // Attach specific handler
    .on("mousemove", mousemove)
    .on("mouseout", mouseout)
    .transition().duration(800).attr("y", (d) => y(d.womenWage)).attr("height", (d) => innerHeight - y(d.womenWage));

  // Add wage labels (keep setTimeout)
  setTimeout(() => {
    g.selectAll(".men-wage-label").data(sortedData).join("text").attr("class", "men-wage-label").attr("x", (d) => x(d.race) + xSubgroup("men") + xSubgroup.bandwidth() / 2).attr("y", (d) => y(d.menWage) - 5).attr("text-anchor", "middle").attr("font-size", "10px").text((d) => `${(d.menWage / 1000).toFixed(0)}k`);
    g.selectAll(".women-wage-label").data(sortedData).join("text").attr("class", "women-wage-label").attr("x", (d) => x(d.race) + xSubgroup("women") + xSubgroup.bandwidth() / 2).attr("y", (d) => y(d.womenWage) - 5).attr("text-anchor", "middle").attr("font-size", "10px").text((d) => `${(d.womenWage / 1000).toFixed(0)}k`);
  }, 900);
